...
interface ScrollableTemperatureChartProps {
  data: ChartData;
  height?: number;
  currentValue?: number;
  timestamp?: string;
  ...
  unit?: string; // Allow customization of temperature unit (°F or °C)
}

const ScrollableTemperatureChart: React.FC<ScrollableTemperatureChartProps> = React.memo(({
  data,
  height = 300,
  currentValue,
  timestamp,
  ...
  const getTemperatureStatus = (temp: number): 'normal' | 'warning' | 'high' => {
    if (temp >= 102.5) return 'high';
    if (temp >= 99.5) return 'warning';
    return 'normal';
  };
  
  // Get temperature status color
  const getStatusColor = (status: 'normal' | 'warning' | 'high'): string => {
    switch (status) {
  ...
  const getFilteredData = () => {
    if (!hasValidData) {
      return {
        ...
        datasets: [{
          data: [],
          color: (opacity = 1) => `rgba(255, 149, 0, ${opacity * 0.8})`, // Orange for temperature
          strokeWidth: 2
        }]
      };
    }
  ...
  // Calculate average temperature from filtered data with NaN protection
  const getAverageTemperature = () => {
    ...
    // For temperature, use 1 decimal place precision
    return Number((sum / filteredDataset.length).toFixed(1));
  };

  // Get temperature status for UI elements
  const temperatureStatus = getTemperatureStatus(derivedCurrentValue);
  const statusColor = getStatusColor(temperatureStatus);
  ...
  return (
    ...
    >
        ...
        <View style={styles.staticRangeContainer}>
          <View>
            ...
            <Text style={styles.rangeValue}>
              {derivedRangeData.min.toFixed(1)}–{derivedRangeData.max.toFixed(1)} <Text style={styles.rangeUnit}>{unit}</Text>
            </Text>
            <Text style={styles.dateRange}>{derivedRangeData.timeRange}</Text>
          </View>
          <View style={styles.averageContainer}>
            <Text style={styles.avgTitle}>AVG</Text>
            <Text style={styles.avgValue}>{getAverageTemperature()} <Text style={styles.rangeUnit}>{unit}</Text></Text>
          </View>
        </View>
          ...
          {hasValidData ? (
            <LineChart
              ...
              chartConfig={{
                ...
                decimalPlaces: 1, // 1 decimal place for temperature
                color: (opacity = 1) => `rgba(255, 149, 0, 1)`, // Orange for temperature
      ...
      <View style={[
        ...
        <Text style={styles.currentValue}>{derivedCurrentValue.toFixed(1)} {unit}</Text>
      </View>
    </Animated.View>
  );
});
...
export default ScrollableTemperatureChart; 