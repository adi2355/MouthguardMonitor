...
interface ScrollableMotionChartProps {
  data: ChartData;
  height?: number;
  currentValue?: number;
  timestamp?: string;
...
const ScrollableMotionChart: React.FC<ScrollableMotionChartProps> = React.memo(({
  data,
  height = 300,
  currentValue,
  timestamp,
  ...
  const getIntensityColor = (intensity: 'low' | 'medium' | 'high'): string => {
    switch (intensity) {
      ...
      default: return '#5856D6'; // Purple for motion (low intensity)
    }
  };
  ...
  const getFilteredData = () => {
    if (!hasValidData) {
      return {
        ...
        datasets: [{
          data: [],
          color: (opacity = 1) => `rgba(88, 86, 214, ${opacity * 0.8})`, // Purple for motion
          strokeWidth: 2
        }]
      };
    }
  ...
  // Calculate average acceleration from filtered data with NaN protection
  const getAverageAcceleration = () => {
    ...
    // For acceleration, use 2 decimal place precision
    return Number((sum / filteredDataset.length).toFixed(2));
  };

  // Get motion intensity for UI elements
  ...
  return (
    ...
    >
        ...
        <View style={styles.staticRangeContainer}>
          <View>
            ...
            <Text style={styles.rangeValue}>
              {derivedRangeData.min.toFixed(2)}–{derivedRangeData.max.toFixed(2)} <Text style={styles.rangeUnit}>g</Text>
            </Text>
            <Text style={styles.dateRange}>{derivedRangeData.timeRange}</Text>
          </View>
          <View style={styles.averageContainer}>
            <Text style={styles.avgTitle}>AVG</Text>
            <Text style={styles.avgValue}>{getAverageAcceleration()} <Text style={styles.rangeUnit}>g</Text></Text>
          </View>
        </View>
      ...
      {/* Current value and peak acceleration indicator */}
      <View style={[
...
export default ScrollableMotionChart; 